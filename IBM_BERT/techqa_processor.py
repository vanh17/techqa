...
def generate_features_for_example(
    ...
    for doc_span_index, doc_span in enumerate(sliding_window_spans):
        ...
        if not span_has_answer and random() > negative_span_subsampling_probability:
            ...
            # Hoang added
            doc_jsons+=(tokens, answer_span_within_context.start, answer_span_within_context.end)
            # end
    # Hoang commented
    # return features
    # Hoang added
...
def create_features(input_query_file: str, input_corpus_file: str, max_seq_length: int,
    ...
    # Hoang comments this line out for preprocessing 
    # features = []
    # end comment out
    # Hoang add list to store documents in json format
    examples = []
    # end initialization of list for storing documents in json format
    ...
    for i, query in enumerate(tqdm(queries, desc='Featurizing queries')):
        ...
        try:
            ...
            if 'ANSWERABLE' in query and query['ANSWERABLE'] == 'Y':
                ...
                # Hoang add this code for for storing documents in json format
                examples.append(generate_features_for_example(
                ...
                # end
                # Hoang comments out this original code for documents json format
                ...
                # end commenting out original code
    ...
    # Hoang commented original codes
    # logging.info('Generated %d features from %d queries' %
    #              (num_generated_features, len(queries)))

    # return features
    # end commenting
    # Hoang add new code for documents json storing
    logging.info('Generated %d features from %d queries' %
                  (len(examples), len(queries)))
...
def get_tech_qa_features(
    ...
    if not path.isfile(cache_file):
        ...
        # Hoang added this code to save file
        doc_json = "datasets/doc_jsons.txt"
        logging.info("Saving features into cached file %s", doc_json)
        with open(doc_json, "w") as doc_json:
            for f in features:
                doc_json.write(str(f)+"\n")
        # end adding
